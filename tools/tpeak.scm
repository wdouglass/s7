...
(define (checks)
  ...
  (let ((previous-version (if (file-exists? "/home/bil/cl_copy/peak-phases.scm")
    ...
    (let ((alls (make-vector 129))
      ...
      (do ((i 2 (+ i 1)))
	  ((> i 128))
	(set! (alls i) (min-peak :all i))
	(set! (odds i) (min-peak :odd i))
	(set! (evens i) (min-peak :even i))
	(set! (primes i) (min-peak :prime i)))
	...
	(let ((old-all (min-peak :all i))
	  ...
	  (if (< old-all (alls i)) (format () ":all ~D: ~A to ~A~%" i old-all (alls i)))
	  (if (< old-odd (odds i)) (format () ":odd ~D: ~A to ~A~%" i old-odd (odds i)))
	  (if (< old-even (evens i)) (format () ":even ~D: ~A to ~A~%" i old-even (evens i)))
	  (if (< old-prime (primes i)) (format () ":prime ~D: ~A to ~A~%" i old-prime (primes i))))))))
