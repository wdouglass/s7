	 ...
	 (mock-number-class
	  (inlet 
	   ...
	   'vector->list     (lambda* (v (start 0) end)
				   ...
				   (let ((i1 (if (mock-number? start)
						 (start 'value)
						 start))
					 (i2 (if (mock-number? end)
						 (end 'value)
						 (or end (length v)))))
				     (if (or (not (integer? i1)) 
					     (not (integer? i2)))
					 (error 'wrong-type-arg start end)
					 (vector->list v i1 i2)))))
	   ...
	   'substring        (lambda* (v (start 0) end)
				   ...
				   (let ((i1 (if (mock-number? start)
					 ...
					 (i2 (if (mock-number? end)
						 ...
						 (or end (length v)))))
				     (if (not (integer? i1))
					 (error 'wrong-type-arg "~S is not an integer" start)
					 (if (not (integer? i2))
					     (error 'wrong-type-arg "~S is not an integer" end)
					     (substring v i1 i2))))))
	   ...
	   'vector-ref       (lambda* (vec ind . indices) 
			       (if (vector? vec)
				   (if (let? ind)
				       (apply #_vector-ref vec (ind 'value) indices)
				       (let loop ((ok-inds ())
						  (inds indices))
					 (if (let? (car inds))
					     (apply #_vector-ref vec ind (append (reverse ok-inds) (cons ((car inds) 'value) (cdr inds))))
					     (loop (cons ok-inds (car inds))
						   (cdr inds)))))
				   (error 'wrong-type-arg "vector-ref ~S ~S~{~^ ~S~}" vec ind indices)))
	   
	   'vector-set!      (lambda* (vec ind . rest)
			       (if (vector? vec)
				   (if (let? ind)
				       (apply #_vector-set! vec (ind 'value) rest)
				       (let loop ((ok-inds ())
						  (inds rest))
					 (if (null? (cdr inds))
					     (apply #_vector-set! vec ind (append (reverse ok-inds) inds))
					     (if (let? (car inds))
						 (apply #_vector-set! vec ind (append (reverse ok-inds) (cons ((car inds) 'value) (cdr inds))))
						 (loop (cons ok-inds (car inds))
						       (cdr inds))))))
				   (error 'wrong-type-arg "vector-set! ~S ~S~{~^ ~S~}" vec ind rest)))
