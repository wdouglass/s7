...
GtkWidget *unite_button(snd_info *sp)   {return(sp->widgets[W_unite]);}

GtkWidget *w_snd_pane(snd_info *sp)     {return(sp->widgets[W_pane]);}
GtkWidget *w_snd_pane_box(snd_info *sp) {return(sp->widgets[W_pane_box]);}

#define SND_PANE(Sp)             Sp->widgets[W_pane]
#define PANE_BOX(Sp)             Sp->widgets[W_pane_box]

#if (!GTK_CHECK_VERSION(3, 92, 1))      
#define NAME_EVENT_BOX(Sp)       Sp->widgets[W_name_event]
#define AMP_EVENT_BOX(Sp)        Sp->widgets[W_amp_event]
#define SPEED_EVENT_BOX(Sp)      Sp->widgets[W_speed_event]
#define SPEED_LABEL_EVENT_BOX(Sp) Sp->widgets[W_speed_label_event]
#define EXPAND_EVENT_BOX(Sp)     Sp->widgets[W_expand_event]
#define CONTRAST_EVENT_BOX(Sp)   Sp->widgets[W_contrast_event]
#define REVSCL_EVENT_BOX(Sp)     Sp->widgets[W_revscl_event]
#define REVLEN_EVENT_BOX(Sp)     Sp->widgets[W_revlen_event]
#endif

#define NAME_HBOX(Sp)            Sp->widgets[W_name_form]
#define NAME_BUTTON(Sp)          Sp->widgets[W_name]
#define NAME_SEPARATOR(Sp)       Sp->widgets[W_amp_sep]

#define CLOSE_BUTTON(Sp)         Sp->widgets[W_close]

#define STATUS_AREA(Sp)          Sp->widgets[W_info]
#define NAME_PIX(Sp)             Sp->widgets[W_name_pix]
#define STOP_PIX(Sp)             Sp->widgets[W_stop_pix]
...
#define CONTROL_PANEL(Sp)        Sp->widgets[W_control_panel]

#define AMP_HBOX(Sp)             Sp->widgets[W_amp_form]
...
#define SPEED_HBOX(Sp)           Sp->widgets[W_speed_form]
#define SPEED_ARROW(Sp)          Sp->widgets[W_speed_pix]
...
#define EXPAND_HBOX(Sp)          Sp->widgets[W_expand_form]
...
#define CONTRAST_HBOX(Sp)        Sp->widgets[W_contrast_form]
...
#define REVERB_RIGHT_BUTTON(Sp)  Sp->widgets[W_reverb_button]
#define REVERB_HBOX(Sp)          Sp->widgets[W_reverb_form]

#define FILTER_HBOX(Sp)          Sp->widgets[W_filter_form]
#define FILTER_LEFT_BUTTON(Sp)   Sp->widgets[W_filter_label]
#define FILTER_ORDER_TEXT(Sp)    Sp->widgets[W_filter_order]
#define FILTER_COEFFS_TEXT(Sp)   Sp->widgets[W_filter]
#define FILTER_RIGHT_BUTTON(Sp)  Sp->widgets[W_filter_button]
...
#define FILTER_ENV(Sp)           Sp->widgets[W_filter_env]
  ...
  if (mini_lock)
    {
      ...
      if (sp->name_pix_cr)
	{
	  cairo_set_source_surface(sp->name_pix_cr, mini_lock, 0, 0);
...
#if GTK_CHECK_VERSION(3, 89, 0)
static void stop_pix_expose(GtkDrawingArea *w, cairo_t *cr, int width, int height, gpointer data)
{
...
static void show_happy_face(chan_info *cp)
{
  mus_float_t pct;
  cairo_t *cr;
...
static void hide_happy_face(chan_info *cp)
{
  cairo_t *cr;
#if (GTK_CHECK_VERSION(3, 89, 0))
  cr = cp->clock_pix_cr;
...
#if GTK_CHECK_VERSION(3, 89, 0)
static void name_pix_expose(GtkDrawingArea *w, cairo_t *cr, int width, int height, gpointer data)
{
  snd_info *sp = (snd_info *)data;
  sp->name_pix_cr = cr;
  if ((has_widgets(sp)) &&
	...
	cairo_set_source_surface(cr, mini_lock, 0, 0);
      else cairo_set_source_surface(cr, blank, 0, 0);
      cairo_paint(cr);
    }
}
#else
...
#if GTK_CHECK_VERSION(3, 89, 0)
static void clock_pix_expose(GtkDrawingArea *w, cairo_t *cr, int width, int height, gpointer data)
{
  ...
  if ((has_widgets(sp)) &&
	...
	{
	  cairo_set_source_surface(cr, blank, 0, 0);
	  cairo_paint(cr);
	}
    }
}
...
#if WITH_AUDIO
static GdkModifierType last_play_state;
...
static gboolean play_button_callback(GtkWidget *w, GdkEventButton *ev, gpointer data)
{
  ...
  gdk_event_get_state((GdkEvent *)ev, &last_play_state);
#else
...
static void play_button_click_callback(GtkWidget *w, gpointer data)
{
  ...
  ss->tracking = ((with_tracking_cursor(ss) != DONT_TRACK) ||
		  ((on) && (last_play_state & (ControlMask | MetaMask))));
...
static GdkModifierType last_sync_state;

static gboolean sync_button_callback(GtkWidget *w, GdkEventButton *ev, gpointer data)
{
#if GTK_CHECK_VERSION(3, 0, 0)
  gdk_event_get_state((GdkEvent *)ev, &last_sync_state);
#else
...
static void sync_button_click(GtkWidget *w, gpointer data)
{
  ...
  on = (bool)(TOGGLE_BUTTON_ACTIVE(w));
  if (on)
    if (last_sync_state & ControlMask) 
      if (last_sync_state & MetaMask)
	if (last_sync_state & ShiftMask)
	  sp->sync = 4;
	else sp->sync = 3;
      else sp->sync = 2;
    else sp->sync = 1;
...
static GdkModifierType last_combine_state;
...
#if GTK_CHECK_VERSION(3, 0, 0)
  gdk_event_get_state((GdkEvent *)ev, &last_combine_state);
#else
...
static void unite_button_click(GtkWidget *w, gpointer data)
{
  ...
  if (on)
    {
      if (last_combine_state & (ControlMask | MetaMask)) 
...
#if GTK_CHECK_VERSION(3, 89, 0)
static void speed_arrow_expose(GtkDrawingArea *w, cairo_t *cr, int width, int height, gpointer data)
{
  snd_info *sp = (snd_info *)data;
  sp->speed_arrow_cr = cr;
  draw_speed_arrow(sp);
}
#else
...
static gboolean filter_drawer_button_press(GtkWidget *w, GdkEventButton *ev, gpointer data)
{
  ...
  edp = sp->flt;
  edp->in_dB = sp->filter_control_in_dB;
  if (env_editor_button_press(edp, (int)x, (int)y, event_time(ev), sp->filter_control_envelope))
    display_filter_env(sp);
  return(false);
}
...
#if GTK_CHECK_VERSION(3, 89, 0)
static void filter_drawer_expose(GtkDrawingArea *w, cairo_t *cr, int width, int height, gpointer data)
{
  snd_info *sp = (snd_info *)data;
  sp->filter_drawer_cr = cr;
  cairo_set_line_width(cr, 1.0);
  display_filter_env(sp);
}
#else
...
snd_info *add_sound_window(char *filename, read_only_t read_only, file_info *hdr)
{
  ...
  if (!sp->widgets)
    {
      sp->widgets = (GtkWidget **)calloc(SP_NUM_WIDGETS, sizeof(GtkWidget *));
  ...
  if (make_widgets)
    {
      ...
      container_set_border_width(SND_PANE(sp), 4); /* this is the outer margin of each sound's box -- 6 seems slightly large */
      ...
      if (sound_style(ss) == SOUNDS_IN_SEPARATE_WINDOWS)
	{
	  ...
	  window_add(sp->dialog, SND_PANE(sp));
      ...
      else
	{
	  ...
	  else box_pack_start(sound_pane_box(ss), SND_PANE(sp), true, true, 0);
	  /* child2 is listener */
	}

      PANE_BOX(sp) = gtk_vbox_new(false, 0);
      paned_set_first(SND_PANE(sp), PANE_BOX(sp), false, false);
      gtk_widget_show(PANE_BOX(sp));

      NAME_HBOX(sp) = gtk_hbox_new(false, 0);
      box_pack_end(PANE_BOX(sp), NAME_HBOX(sp), false, false, 0);
	...
	add_channel_window(sp, i, chan_min_y, 0, NULL, WITH_FW_BUTTONS, WITH_EVENTS);
      ...
      paned_set_second(SND_PANE(sp), CONTROL_PANEL(sp), false, true); /* add2 but resize=false */
      ...
      button_set_relief(CLOSE_BUTTON(sp), GTK_RELIEF_NONE);
#if (!GTK_CHECK_VERSION(3, 92, 1))
      gtk_button_set_image(GTK_BUTTON(CLOSE_BUTTON(sp)), image_new_with_icon(ICON_CLOSE, GTK_ICON_SIZE_MENU));
#endif
      box_pack_start(NAME_HBOX(sp), CLOSE_BUTTON(sp), false, false, 8);
      ...
      box_pack_start(NAME_HBOX(sp), NAME_EVENT_BOX(sp), false, false, 5);
...
#if (!GTK_CHECK_VERSION(3, 92, 1))
      container_add(NAME_EVENT_BOX(sp), NAME_BUTTON(sp));
#else
      box_pack_start(NAME_HBOX(sp), NAME_BUTTON(sp), false, false, 5);
      ...
      box_pack_start(NAME_HBOX(sp), NAME_PIX(sp), false, false, 2);
      gtk_widget_show(NAME_PIX(sp));
...
#if (GTK_CHECK_VERSION(3, 89, 0))
      gtk_drawing_area_set_content_width(GTK_DRAWING_AREA(NAME_PIX(sp)), gtk_widget_get_allocated_width(NAME_PIX(sp)));
      gtk_drawing_area_set_content_height(GTK_DRAWING_AREA(NAME_PIX(sp)), gtk_widget_get_allocated_height(NAME_PIX(sp)));
      gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(NAME_PIX(sp)), name_pix_expose, (void *)sp, NULL);
#else
      ...
      box_pack_start(NAME_HBOX(sp), STOP_PIX(sp), false, false, 2);
      gtk_widget_show(STOP_PIX(sp));
...
#if (GTK_CHECK_VERSION(3, 89, 0))
      gtk_drawing_area_set_content_width(GTK_DRAWING_AREA(STOP_PIX(sp)), gtk_widget_get_allocated_width(STOP_PIX(sp)));
      gtk_drawing_area_set_content_height(GTK_DRAWING_AREA(STOP_PIX(sp)), gtk_widget_get_allocated_height(STOP_PIX(sp)));
      gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(STOP_PIX(sp)), stop_pix_expose, (void *)sp, NULL);
#endif
	...
	for (i = 0; i < sp->nchans; i++)
	  {
	    ...
	    box_pack_start(NAME_HBOX(sp), CLOCK_PIX(sp, i), false, false, 2);
...
#if (GTK_CHECK_VERSION(3, 89, 0))
	    gtk_drawing_area_set_content_width(GTK_DRAWING_AREA(CLOCK_PIX(sp, i)), gtk_widget_get_allocated_width(CLOCK_PIX(sp, i)));
	    gtk_drawing_area_set_content_height(GTK_DRAWING_AREA(CLOCK_PIX(sp, i)), gtk_widget_get_allocated_height(CLOCK_PIX(sp, i)));
	    gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(CLOCK_PIX(sp, i)), clock_pix_expose, sp->chans[i], NULL);
#else
...
#endif
      box_pack_start(NAME_HBOX(sp), STATUS_AREA(sp), true, true, 2);
      ...
      box_pack_end(NAME_HBOX(sp), PLAY_BUTTON(sp), false, false, BUTTON_SPACE); /* need space here or "play" hits window edge */
      ...
      box_pack_end(NAME_HBOX(sp), SYNC_BUTTON(sp), false, false, BUTTON_SPACE);
      ...
      box_pack_end(NAME_HBOX(sp), UNITE_BUTTON(sp), false, false, BUTTON_SPACE);
      ...
      box_pack_start(CONTROL_PANEL(sp), NAME_SEPARATOR(sp), false, false, 4);
      gtk_widget_show(NAME_SEPARATOR(sp));
      
      AMP_HBOX(sp) = gtk_hbox_new(false, 2);
      box_pack_start(CONTROL_PANEL(sp), AMP_HBOX(sp), false, false, 0);

#if (!GTK_CHECK_VERSION(3, 92, 1))      
      AMP_EVENT_BOX(sp) = gtk_event_box_new();
      box_pack_start(AMP_HBOX(sp), AMP_EVENT_BOX(sp), false, false, 4);
...
#if (!GTK_CHECK_VERSION(3, 92, 1))      
      container_add(AMP_EVENT_BOX(sp), AMP_BUTTON(sp));
#else
      box_pack_start(AMP_HBOX(sp), AMP_BUTTON(sp), false, false, 4);
      ...
      box_pack_start(AMP_HBOX(sp), AMP_LABEL(sp), false, false, 0);
      ...
      box_pack_start(AMP_HBOX(sp), AMP_SCROLLBAR(sp), true, true, 4);
      ...
      box_pack_start(CONTROL_PANEL(sp), SPEED_HBOX(sp), false, false, 0);
      
#if (!GTK_CHECK_VERSION(3, 92, 1))      
      SPEED_EVENT_BOX(sp) = gtk_event_box_new();
      box_pack_start(SPEED_HBOX(sp), SPEED_EVENT_BOX(sp), false, false, 4);
...
#if (!GTK_CHECK_VERSION(3, 92, 1))      
      container_add(SPEED_EVENT_BOX(sp), SPEED_BUTTON(sp));
#else
      box_pack_start(SPEED_HBOX(sp), SPEED_BUTTON(sp), false, false, 4);
      ...
      box_pack_start(SPEED_HBOX(sp), SPEED_LABEL_EVENT_BOX(sp), false, false, 4);
	...
	default:                        SPEED_LABEL(sp) = gtk_label_new("1.00 "); break;
...
#if (!GTK_CHECK_VERSION(3, 92, 1))      
      container_add(SPEED_LABEL_EVENT_BOX(sp), SPEED_LABEL(sp));
#else
      box_pack_start(SPEED_HBOX(sp), SPEED_LABEL(sp), false, false, 4);
      ...
      box_pack_start(SPEED_HBOX(sp), SPEED_SCROLLBAR(sp), true, true, 4);
...
#endif
      box_pack_start(SPEED_HBOX(sp), SPEED_ARROW(sp), false, false, 2);
...
#if (GTK_CHECK_VERSION(3, 89, 0))
      gtk_drawing_area_set_content_width(GTK_DRAWING_AREA(SPEED_ARROW(sp)), gtk_widget_get_allocated_width(SPEED_ARROW(sp)));
      gtk_drawing_area_set_content_height(GTK_DRAWING_AREA(SPEED_ARROW(sp)), gtk_widget_get_allocated_height(SPEED_ARROW(sp)));
      gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(SPEED_ARROW(sp)), speed_arrow_expose, (void *)sp, NULL);
#else
      ...
      box_pack_start(CONTROL_PANEL(sp), EXPAND_HBOX(sp), false, false, 0);
      
#if (!GTK_CHECK_VERSION(3, 92, 1))      
      EXPAND_EVENT_BOX(sp) = gtk_event_box_new();
      box_pack_start(EXPAND_HBOX(sp), EXPAND_EVENT_BOX(sp), false, false, 4);
...
#if (!GTK_CHECK_VERSION(3, 92, 1))      
      container_add(EXPAND_EVENT_BOX(sp), EXPAND_LEFT_BUTTON(sp));
#else
      box_pack_start(EXPAND_HBOX(sp), EXPAND_LEFT_BUTTON(sp), false, false, 4);
      ...
      box_pack_start(EXPAND_HBOX(sp), EXPAND_LABEL(sp), false, false, 0);
      ...
      box_pack_start(EXPAND_HBOX(sp), EXPAND_SCROLLBAR(sp), true, true, 4);
      ...
      box_pack_start(EXPAND_HBOX(sp), EXPAND_RIGHT_BUTTON(sp), false, false, 3);
      ...
      box_pack_start(CONTROL_PANEL(sp), CONTRAST_HBOX(sp), false, false, 0);
#if (!GTK_CHECK_VERSION(3, 92, 1))            
      CONTRAST_EVENT_BOX(sp) = gtk_event_box_new();
      box_pack_start(CONTRAST_HBOX(sp), CONTRAST_EVENT_BOX(sp), false, false, 4);
...
#if (!GTK_CHECK_VERSION(3, 92, 1))      
      container_add(CONTRAST_EVENT_BOX(sp), CONTRAST_LEFT_BUTTON(sp));
#else
      box_pack_start(CONTRAST_HBOX(sp), CONTRAST_LEFT_BUTTON(sp), false, false, 4);
      ...
      box_pack_start(CONTRAST_HBOX(sp), CONTRAST_LABEL(sp), false, false, 0);
      ...
      box_pack_start(CONTRAST_HBOX(sp), CONTRAST_SCROLLBAR(sp), true, true, 4);
      ...
      box_pack_start(CONTRAST_HBOX(sp), CONTRAST_RIGHT_BUTTON(sp), false, false, 3);
      ...
      box_pack_start(CONTROL_PANEL(sp), REVERB_HBOX(sp), false, false, 0);
      
#if (!GTK_CHECK_VERSION(3, 92, 1))      
      REVSCL_EVENT_BOX(sp) = gtk_event_box_new();
      box_pack_start(REVERB_HBOX(sp), REVSCL_EVENT_BOX(sp), false, false, 4);
...
#if (!GTK_CHECK_VERSION(3, 92, 1))      
      container_add(REVSCL_EVENT_BOX(sp), REVSCL_BUTTON(sp));
#else
      box_pack_start(REVERB_HBOX(sp), REVSCL_BUTTON(sp), false, false, 4);
      ...
      box_pack_start(REVERB_HBOX(sp), REVSCL_LABEL(sp), false, false, 0);
      ...
      box_pack_start(REVERB_HBOX(sp), REVSCL_SCROLLBAR(sp), true, true, 4);
      ...
      box_pack_start(REVERB_HBOX(sp), REVLEN_EVENT_BOX(sp), false, false, 4);
...
#if (!GTK_CHECK_VERSION(3, 92, 1))            
      container_add(REVLEN_EVENT_BOX(sp), REVLEN_BUTTON(sp));
#else
      box_pack_start(REVERB_HBOX(sp), REVLEN_BUTTON(sp), false, false, 4);
      ...
      box_pack_start(REVERB_HBOX(sp), REVLEN_LABEL(sp), false, false, 0);
      ...
      box_pack_start(REVERB_HBOX(sp), REVLEN_SCROLLBAR(sp), true, true, 4);
      ...
      box_pack_start(REVERB_HBOX(sp), REVERB_RIGHT_BUTTON(sp), false, false, 3);
      ...
      box_pack_start(CONTROL_PANEL(sp), FILTER_HBOX(sp), false, false, 0);
      
      FILTER_LEFT_BUTTON(sp) = gtk_label_new("filter:");
      box_pack_start(FILTER_HBOX(sp), FILTER_LEFT_BUTTON(sp), false, false, 4);
      ...
      box_pack_start(FILTER_HBOX(sp), FILTER_ORDER_TEXT(sp), false, false, 2);
      ...
      box_pack_start(FILTER_HBOX(sp), FILTER_HZ_BUTTON(sp), false, false, 2);
      ...
      box_pack_start(FILTER_HBOX(sp), FILTER_DB_BUTTON(sp), false, false, 2);
      ...
      box_pack_start(FILTER_HBOX(sp), FILTER_RIGHT_BUTTON(sp), false, false, 3);
	...
	box_pack_start(CONTROL_PANEL(sp), fbox, true, true, 0);
	gtk_widget_show(fbox);
	
	/* -------- FILTER GRAPH -------- */
      
	FILTER_FRAME(sp) = gtk_frame_new(NULL);
	box_pack_start(fbox, FILTER_FRAME(sp), true, true, 30);
	...
	container_add(FILTER_FRAME(sp), FILTER_ENV(sp));
	gtk_widget_show(FILTER_ENV(sp));
#if (GTK_CHECK_VERSION(3, 89, 0))
	gtk_drawing_area_set_content_width(GTK_DRAWING_AREA(FILTER_ENV(sp)), gtk_widget_get_allocated_width(FILTER_ENV(sp)));
	gtk_drawing_area_set_content_height(GTK_DRAWING_AREA(FILTER_ENV(sp)), gtk_widget_get_allocated_height(FILTER_ENV(sp)));
	gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(FILTER_ENV(sp)), filter_drawer_expose, (void *)sp, NULL);
#else
...
void snd_info_cleanup(snd_info *sp)
{
  if (has_widgets(sp))
	...
	gtk_widget_hide(sp->dialog);
      ...
      if ((sp->widgets) && (SND_PANE(sp))) gtk_widget_hide(SND_PANE(sp));
			...
			void (*callback)(GtkAdjustment *adj, gpointer context))
{
  ...
  box_pack_start(main, frame, true, true, 10); 
  ...
  if (!controls_dialog)
    {
      ...
      container_set_border_width(controls_dialog, 4);
...
#if HAVE_SCHEME
  return(Xen_cons(s7_make_c_pointer_with_type(s7, SND_PANE(sp), s7_make_symbol(s7, "GtkPaned_"), Xen_false),
	  Xen_cons(s7_make_c_pointer_with_type(s7, NAME_BUTTON(sp), s7_make_symbol(s7, "GtkLabel_"), Xen_false),
           Xen_cons(s7_make_c_pointer_with_type(s7, CONTROL_PANEL(sp), s7_make_symbol(s7, "GtkBox_"), Xen_false),
	    Xen_cons(s7_make_c_pointer_with_type(s7, STATUS_AREA(sp), s7_make_symbol(s7, "GtkLabel_"), Xen_false),
#if WITH_AUDIO
	     Xen_cons(s7_make_c_pointer_with_type(s7, PLAY_BUTTON(sp), s7_make_symbol(s7, "GtkCheckButton_"), Xen_false),
#else
             Xen_cons(Xen_false,
#endif
	      Xen_cons(s7_make_c_pointer_with_type(s7, FILTER_ENV(sp), s7_make_symbol(s7, "GtkDrawingArea_"), Xen_false),
	       Xen_cons(s7_make_c_pointer_with_type(s7, UNITE_BUTTON(sp), s7_make_symbol(s7, "GtkCheckButton_"), Xen_false),
#if (GTK_CHECK_VERSION(3, 89, 0))
		Xen_cons(s7_make_c_pointer_with_type(s7, (void *)(sp->name_pix_cr), s7_make_symbol(s7, "cairo_t_"), Xen_false),
#else
		Xen_cons(Xen_false,
#endif
	         Xen_cons(s7_make_c_pointer_with_type(s7, NAME_PIX(sp), s7_make_symbol(s7, "GtkDrawingArea_"), Xen_false),
		  Xen_cons(s7_make_c_pointer_with_type(s7, PANE_BOX(sp), s7_make_symbol(s7, "GtkBox_"), Xen_false),
		   Xen_cons(s7_make_c_pointer_with_type(s7, NAME_HBOX(sp), s7_make_symbol(s7, "GtkBox_"), Xen_false),
	            Xen_empty_list))))))))))));
#else
...
GtkWidget *snd_entry_new(GtkWidget *container, GtkWidget *prev, snd_entry_bg_t with_white_background)
{
  ...
  else box_pack_start(container, text, true, true, 2);
#else
  box_pack_start(container, text, true, true, 2);
#endif
...
GtkWidget *snd_entry_new_with_size(GtkWidget *container, int size)
{
  ...
  box_pack_start(container, text, false, false, 4);
